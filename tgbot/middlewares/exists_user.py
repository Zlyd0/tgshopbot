...
from tgbot.database.methods.user import get_userx, add_userx, update_userx
from tgbot.misc.const_functions import clear_html
        ...
        if this_user is not None:
            ...
            if get_settings['status_work'] == "False" or this_user.id in get_admins():
                if not this_user.is_bot:
                    get_user = await get_userx(user_id=this_user.id)
                    ...
                    if get_user is None:
                        await add_userx(user_id, user_login.lower(), user_name)
                    else:
                        if user_name != get_user['user_name']:
                            await update_userx(get_user['user_id'], user_name=user_name)

                        if len(user_login) >= 1:
                            if user_login.lower() != get_user['user_login']:
                                await update_userx(get_user['user_id'], user_login=user_login.lower())
                        else:
                            await update_userx(get_user['user_id'], user_login="")
