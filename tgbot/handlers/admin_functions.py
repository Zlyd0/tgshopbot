...
from tgbot.database.methods.purchase import get_purchasex, last_purchasesx
from tgbot.database.methods.refill import get_refillx
from tgbot.database.methods.user import get_userx, get_all_usersx, update_userx
...
from tgbot.misc.const_functions import get_unix, is_number, clear_html
from tgbot.misc.misc_functions import open_profile_admin, upload_text
...
async def functions_profile_get(message: Message, state: FSMContext):
    ...
    if find_user.isdigit():
        get_user = await get_userx(user_id=int(find_user))
    else:
        if find_user.startswith("@"):
            find_user = find_user[1:]
        get_user = await get_userx(user_login=find_user.lower())

    if get_user is not None:
        await state.finish()
        await message.answer(await open_profile_admin(get_user['user_id']),
...
async def functions_receipt_get(message: Message, state: FSMContext):
    ...
    if find_receipt.startswith("#"):
        find_receipt = find_receipt[1:]

    get_refill = await get_refillx(refill_receipt=find_receipt)
    get_purchase = await get_purchasex(purchase_receipt=find_receipt)
    ...
    if get_refill is not None:
        ...
        await message.answer(
            ...
            "➖➖➖➖➖➖➖➖➖➖\n"
            f"👤 Пользователь: <a href='tg://user?id={get_refill['user_id']}'>{get_refill['user_name']}</a> | <code>"
            f"{get_refill['user_id']}</code>\n"
        ...
        await message.answer(
            ...
            f"➖➖➖➖➖➖➖➖➖➖\n"
            f"👤 Пользователь: <a href='tg://user?id={get_purchase['user_id']}'>{get_purchase['user_name']}</a> | <code>"
            f"{get_purchase['user_id']}</code>\n"
...
async def functions_mail_confirm(call: CallbackQuery, state: FSMContext):
    ...
    if get_action == "yes":
        ...
        create_task(functions_mail_make(send_message, call))
    else:
...
async def functions_profile_refresh(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(await open_profile_admin(user_id), reply_markup=profile_search_finl(user_id))
...
async def functions_profile_purchases(call: CallbackQuery, state: FSMContext):
    ...
    last_purchases = await last_purchasesx(int(user_id), 10)
    ...
    if len(last_purchases) >= 1:
        ...
        for purchases in last_purchases:
            ...
            await call.message.answer(f"<b>🧾 Чек: <code>#{purchases['purchase_receipt']}</code></b>\n"
                                      f"🎁 Товар: <code>{purchases['purchase_position_name']} | "
                                      f"{purchases['purchase_count']}шт | {purchases['purchase_price']}₽</code>\n"
...
async def functions_profile_balance_add_get(message: Message, state: FSMContext):
    ...
    await update_userx(user_id, user_balance=get_user['user_balance'] + int(message.text))
    ...
    await message.answer(await open_profile_admin(user_id), reply_markup=profile_search_finl(user_id))
...
async def functions_profile_balance_set_get(message: Message, state: FSMContext):
    ...
    await state.finish()
    get_user = await get_userx(user_id=int(user_id))
    await update_userx(user_id, user_balance=int(message.text))
