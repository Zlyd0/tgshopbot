...
from tgbot.database.methods.refill import get_refillx, add_refillx
from tgbot.database.methods.user import get_userx, update_userx
from tgbot.keyboards.inline_user import refill_choice_finl, refill_bill_finl
...
async def refill_way(call: CallbackQuery, state: FSMContext):
    get_kb = await refill_choice_finl()
    ...
    if message.text.isdigit():
        ...
        if min_input_qiwi <= pay_amount <= 300000:
            ...
            await cache_message.edit_text(f"<b>❌ Неверная сумма пополнения</b>\n"
                                          f"▶ Cумма не должна быть меньше <code>{min_input_qiwi}₽</code> и больше "
                                          f"<code>300 000₽</code>\n"
...
async def refill_check_form(call: CallbackQuery):
    ...
    if pay_status == "PAID":
        get_refill = await get_refillx(refill_receipt=receipt)
...
async def refill_success(call: CallbackQuery, receipt, amount, get_way):
    ...
    await add_refillx(get_user['user_id'], get_user['user_login'], get_user['user_name'], receipt,
                      amount, receipt, get_way, get_date(), get_unix())

    await update_userx(call.from_user.id,
    ...
    await send_admins(
        f"👤 Пользователь: <b>@{get_user['user_login']}</b> | <a href='tg://user?id={get_user['user_id']}'>"
        f"{get_user['user_name']}</a> | <code>{get_user['user_id']}</code>\n"
