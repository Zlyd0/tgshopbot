...
from aiogram.utils.exceptions import MessageCantBeDeleted, MessageToDeleteNotFound, CantParseEntities

from tgbot.database.methods.category import add_categoryx, get_categoryx, get_all_categoriesx, update_categoryx, \
    remove_categoryx, clear_categoryx
from tgbot.database.methods.item import remove_itemx, get_all_itemsx, clear_itemx, get_itemsx, add_itemx, get_itemx
from tgbot.database.methods.position import get_positionsx, get_all_positionsx, remove_positionx, clear_positionx, \
    add_positionx, update_positionx, get_positionx
from tgbot.database.methods.user import get_userx
from tgbot.filters.all_filters import IsAdmin
from tgbot.keyboards.inline_admin import category_edit_open_finl, category_edit_cancel_finl, \
    category_edit_delete_finl, position_edit_open_finl, position_edit_cancel_finl, position_edit_delete_finl, \
...
from tgbot.keyboards.inline_page import category_edit_swipe_fp, position_create_swipe_fp, \
    position_edit_category_swipe_fp, products_add_category_swipe_fp, position_edit_swipe_fp, \
    products_add_position_swipe_fp
from tgbot.keyboards.reply_all import items_frep, finish_load_rep
from tgbot.loader import dp
from tgbot.middlewares.throttling import rate_limit
from tgbot.misc.const_functions import get_unix, clear_html, clear_list
from tgbot.misc.misc_functions import get_position_admin, upload_text
...
async def product_position_create(message: Message, state: FSMContext):
    ...
    if len(await get_all_categoriesx()) >= 1:
        await message.answer("<b>📁 Выберите категорию для позиции</b>",
                             reply_markup=await position_create_swipe_fp(0))
    else:
...
async def product_item_create(message: Message, state: FSMContext):
    ...
    if len(await get_all_positionsx()) >= 1:
        await message.answer("<b>🎁 Выберите категорию с нужной позицией</b>",
                             reply_markup=await products_add_category_swipe_fp(0))
    else:
...
async def product_item_delete(message: Message, state: FSMContext):
    ...
    await message.answer("<b>🖍 Вводите айди товаров, которые нужно удалить</b>\n"
                         ...
                         "❕ Если хотите удалить несколько товаров, отправьте ID товаров через запятую или пробел. "
                         "Пример:\n"
...
async def product_category_create_name(message: Message, state: FSMContext):
    if len(message.text) <= 50:
        ...
        await add_categoryx(category_id, clear_html(message.text))

        await state.finish()

        get_positions = len(await get_positionsx(category_id=category_id))
...
async def product_category_edit_open(call: CallbackQuery, state: FSMContext):
    ...
    get_positions = len(await get_positionsx(category_id=int(category_id)))
...
async def product_category_edit_name_get(message: Message, state: FSMContext):
    ...
    if len(message.text) <= 50:
        ...
        await update_categoryx(category_id, category_name=clear_html(message.text))
...
async def product_category_edit_delete_confirm(call: CallbackQuery, state: FSMContext):
    ...
    if get_action == "yes":
        await remove_categoryx(category_id=int(category_id))
        await remove_positionx(category_id=int(category_id))
        await remove_itemx(category_id=int(category_id))
        ...
        get_fat_count = len(await get_positionsx(category_id=int(category_id)))
        get_category = await get_categoryx(category_id=int(category_id))
...
async def product_category_remove_confirm(call: CallbackQuery, state: FSMContext):
    ...
    if get_action == "yes":
        get_categories = len(await get_all_categoriesx())
...
async def product_position_create_swipe(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text("<b>📁 Выберите категорию для позиции ➕</b>",
                                 reply_markup=await position_create_swipe_fp(remover))
...
async def product_position_create_photo(message: Message, state: FSMContext):
    ...
    await add_positionx(position_id, position_name, int(position_price), position_description, position_photo,
                        int(category_id))
    get_message, get_photo = await get_position_admin(position_id)
...
async def product_position_edit_name_get(message: Message, state: FSMContext):
    async with state.proxy() as data:
        position_id = int(data['here_cache_position_id'])
    ...
    if len(message.text) <= 50:
        ...
        await update_positionx(position_id, position_name=clear_html(message.text))
        get_message, get_photo = await get_position_admin(position_id)
...
async def product_position_edit_price_get(message: Message, state: FSMContext):
    ...
    if message.text.isdigit():
        if 0 <= int(message.text) <= 10000000:
            ...
            await update_positionx(position_id, position_price=int(message.text))
            get_message, get_photo = await get_position_admin(position_id)
...
async def product_position_edit_description_get(message: Message, state: FSMContext):
    ...
    try:
        if len(message.text) <= 600:
            ...
            await update_positionx(position_id, position_description=message.text)
            get_message, get_photo = await get_position_admin(position_id)
...
async def product_position_edit_photo_get(message: Message, state: FSMContext):
    ...
    await update_positionx(position_id, position_photo=position_photo)
    get_message, get_photo = await get_position_admin(position_id)
...
async def product_position_edit_delete_confirm(call: CallbackQuery, state: FSMContext):
    ...
    if get_action == "yes":
        ...
        if len(await get_positionsx(category_id=category_id)) >= 1:
            await call.message.edit_text("<b>📁 Выберите нужную вам позицию 🖍</b>",
                                         reply_markup=await position_edit_swipe_fp(remover, category_id))
        else:
            with suppress(MessageCantBeDeleted):
...
async def product_item_category_open(call: CallbackQuery, state: FSMContext):
    ...
    if len(get_positions) >= 1:
        await call.message.answer("<b>🎁 Выберите нужную вам позицию</b>",
                                  reply_markup=await products_add_position_swipe_fp(0, category_id))
    else:
        await call.answer(f"🎁 Позиции в категории {get_category['category_name']} отсутствуют")
...
async def product_item_category_swipe(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text("<b>🎁 Выберите категорию с нужной позицией</b>",
                                 reply_markup=await products_add_category_swipe_fp(remover))
...
async def product_item_position_swipe(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text("<b>🎁 Выберите нужную вам позицию</b>",
                                 reply_markup=await products_add_position_swipe_fp(remover, category_id))
...
async def product_item_load_get(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        category_id = int(data['here_cache_add_item_category_id'])
        position_id = int(data['here_cache_add_item_position_id'])
        data['here_count_add_items'] += count_add

    get_user = await get_userx(user_id=message.from_user.id)
    await add_itemx(category_id, position_id, get_all_items, get_user['user_id'], get_user['user_name'])
...
async def product_item_delete_get(message: Message, state: FSMContext):
    ...
    for item_id in save_ids:
        check_item = await get_itemx(item_id=int(item_id))
        if check_item is not None:
            await remove_itemx(item_id=int(item_id))
