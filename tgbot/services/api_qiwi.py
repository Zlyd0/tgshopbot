...
class QiwiAPI(AsyncClass):
    async def __ainit__(self, dp, login=None, token=None, secret=None, add_pass=False,
        ...
        if login is not None:
            ...
            self.login = (await get_paymentx())['qiwi_login']
            self.token = (await get_paymentx())['qiwi_token']
            self.secret = (await get_paymentx())['qiwi_secret']

        self.base_url = "https://edge.qiwi.com/{}/{}/persons/{}/{}"
        self.headers = {"authorization": f"Bearer {self.token}"}
        self.nickname = (await get_paymentx())['qiwi_nickname']
    ...
    async def pre_checker(self):
        if self.login != "None":
            ...
            if self.add_pass:
                ...
                if status:
                    await update_paymentx(qiwi_login=self.login, qiwi_token=self.token, qiwi_secret=self.secret)
                else:
                    return False
    ...
    async def check_account(self):
        ...
        if status_history and status_balance:
            ...
            if 400 in [code_history, code_balance]:
                ...
                return_message = ded(f""" <b>🥝 Введённые QIWI данные не прошли проверку ❌</b> ▶ Код ошибки: 
                <code>CERTIFICATE_VERIFY_FAILED certificate verify failed: self signed certificate in certificate 
                chain</code> ❗ Ваш сервер/дедик/устройство блокирует запросы к QIWI. Отключите антивирус или другие 
                блокирующие ПО. """)
            else:
                return_message = ded(f"""
                                 <b>🥝 Введённые QIWI данные не прошли проверку ❌</b>\n
    ...
    async def bill_pay(self, get_amount, get_way):
        ...
        if response:
            ...
            if get_way == "Form":
                ...
                bill_url = f"https://qiwi.com/payment/form/99?extra%5B%27account%27%5D={self.login}&amountInteger=" \
                           f"{get_amount}&amountFraction=0&extra%5B%27comment%27%5D={bill_receipt}" \
                ...
                bill_url = f"https://qiwi.com/payment/form/99999?amountInteger={get_amount}&amountFraction=0&currency" \
                           f"=643&extra%5B%27comment%27%5D={bill_receipt}&extra%5B%27acc" \
                           f"ount%27%5D={self.nickname}&blocked%5B0%5D=comment&blocked%5B1%5D=account&blocked%5B2%5D" \
                           f"=sum&0%5Bextra%5B%27accountType%27%5D%5D=nickname"
    ...
    async def _request(self, action, version, get_way, params=None):
        ...
        try:
            ...
            return True, loads((await response.read()).decode()), response.status
